<?php
	...
	if (!isset($_SESSION['mod_id']) && $_SESSION['mod_id']==null) {
		header("location: ../");
	}
	...
	if(isset($_POST['token'])){
		$update=mysqli_query($con,"SELECT * from ticket where id=$id");
		if ($update) {
			$success=sha1(md5("datos seleccionados con exito"));
            header("location: ../atender_casos.php?success=$success");
	   	}else{
	   		// echo "error";
	   	}
